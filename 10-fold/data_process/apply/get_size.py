...
# @Last Modified time: 2020-11-18 19:25:14
import pandas as pd
...
def new_extract(num):
    ...
    tcp["count"] = grouped.transform(len)

    for pkt_num in range(PACKET_NUMBER):
        temp_df = tcp[tcp["count"] == PACKET_NUMBER]
        temp_grouped = temp_df.groupby(["srcIP", "srcPort", "dstIP", "dstPort", "protocol"])
